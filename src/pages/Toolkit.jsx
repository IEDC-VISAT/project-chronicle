...
function Toolkit() {
  const { data: templates, loading: loadingTemplates } = useApi('/toolkit/templates/');
  const { data: prompts, loading: loadingPrompts } = useApi('/toolkit/prompts/');
  const { data: linkedInFields, loading: loadingLinkedIn } = useApi('/toolkit/linkedin/');

  const [selectedField, setSelectedField] = useState('');
  const [generatedContent, setGeneratedContent] = useState(null);
  const [copiedPrompt, setCopiedPrompt] = useState(null);

  const loading = loadingTemplates || loadingPrompts || loadingLinkedIn;

  const handleGenerateLinkedIn = () => {
    const found = linkedInFields.find(f => f.field_name === selectedField);
    if (found) setGeneratedContent(found);
  };
    ...
    <main className="max-w-6xl mx-auto px-4 py-8">
      ...
      {loading ? (
        <div className="retro-card text-center py-12">
          <p className="font-sans text-lg text-retro-brown animate-pulse">Loading toolkit...</p>
        </div>
      ) : (
        <>
              ...
              {templates.map(template => (
                <div key={template.id} className="retro-card">
                  ...
                  <div className="mb-3 p-3 bg-retro-beige border-2 border-retro-black">
                    <p className="font-sans text-xs mb-2"><strong>Style:</strong> {template.style}</p>
                    <p className="font-sans text-xs"><strong>Best For:</strong> {template.best_for}</p>
                  </div>
                  <button className="retro-btn-primary w-full">DOWNLOAD TEMPLATE</button>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="mb-8">
            ...
            <div className="retro-card">
              ...
              <div className="mb-4">
                <label className="font-sans text-sm font-bold block mb-2">Select Your Field:</label>
                <select
                  value={selectedField}
                ...
                <div className="border-t-2 border-retro-black pt-4 mt-4">
                  <div className="mb-4">
                    <h3 className="font-sans font-bold mb-2 flex items-center justify-between">
                      ...
                      <button onClick={() => navigator.clipboard.writeText(generatedContent.headline)} className="retro-btn text-xs">COPY</button>
                    </h3>
                    <div className="p-3 bg-retro-beige border-2 border-retro-black font-sans text-sm">{generatedContent.headline}</div>
                  </div>
                  <div>
                    <h3 className="font-sans font-bold mb-2 flex items-center justify-between">
                      <span>📝 Suggested About Section:</span>
                      <button onClick={() => navigator.clipboard.writeText(generatedContent.about)} className="retro-btn text-xs">COPY</button>
                    </h3>
                    <div className="p-3 bg-retro-beige border-2 border-retro-black font-sans text-sm whitespace-pre-line max-h-96 overflow-y-auto">{generatedContent.about}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {prompts.map(prompt => (
                <div key={prompt.id} className="retro-card">
                  ...
                  <p className="font-sans text-xs text-retro-brown italic mb-3">💡 {prompt.usage}</p>
                  <button
                    onClick={() => handleCopyPrompt(prompt.id, prompt.prompt_text)}
