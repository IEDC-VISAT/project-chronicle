...
function SkillDetail() {
  ...
  const rawSkill = skills.find(s => s.id === parseInt(id));
  if (!rawSkill) {
    return (
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="retro-card text-center py-12">
  ...
  const skill = {
    ...rawSkill,
    editorsPick: rawSkill.editors_pick,
    howToLearn: rawSkill.how_to_learn || [],
    whereToLearn: rawSkill.where_to_learn || [],
  };
  ...
  const getDifficultyColor = (difficulty) => {
    switch (difficulty) {
      ...
      case 'Advanced': return 'bg-retro-red';
      default: return 'bg-retro-brown';
    }
  };
    ...
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      <article className="retro-card">
        ...
        <div className="border-b-4 border-retro-black pb-4 mb-6">
          ...
          <p className="font-sans text-lg leading-relaxed">{skill.description}</p>

          <div className="flex flex-wrap gap-2 mt-4">
            {(skill.tags || []).map((tag, index) => (
              <span key={index} className="inline-block px-3 py-1 text-sm font-sans border-2 border-retro-black bg-retro-beige">
                {tag}
              </span>
            ))}
          </div>
            ...
            <ol className="font-sans space-y-3 list-none pl-0">
              {skill.howToLearn.map((step, index) => (
                <li key={index} className="flex items-start">
                  <span className="inline-flex w-8 h-8 border-2 border-retro-black bg-retro-blue text-white items-center justify-center font-bold mr-3 flex-shrink-0">
                    {index + 1}
                  </span>
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {skill.whereToLearn.map((resource, index) => (
              <div key={index} className="border-2 border-retro-black p-4 bg-retro-white hover:bg-retro-beige transition-colors">
                ...
                <a href={resource.url} target="_blank" rel="noopener noreferrer" className="font-sans text-sm text-retro-blue underline">
                  Visit Resource →
                </a>
              </div>
            ))}
        ...
        <div className="border-t-4 border-retro-black pt-6">
          <div className="flex flex-wrap gap-4 justify-center items-center">
            <Link to="/roadmap" className="retro-btn-primary">VIEW CAREER ROADMAPS</Link>
            <Link to="/toolkit" className="retro-btn">EXPLORE TOOLKIT</Link>
            <button
